...
export default function OtherProfile_Mobile() {
    ...
    useEffect(() => {
        const fetchdp = async () => {
            ...
            const docRef = doc(db, "User Details", useruid);
            ...
            if (user) {
                ...
                if (snapshot.exists()) {
                    ...
                    for (const postId of postids) {
                        ...
                        if (docSnap.exists()) {
                            if (docSnap.data()['Uploaded UID'] === useruid) {
    ...
    useEffect(() => {
        const fetchfollowers = async () => {
            var useruid=localStorage.getItem('clickeduid');
            const Uid = auth.currentUser.uid;
            const docsnap = doc(db, 'Followers', useruid);
            const snapshot = await getDoc(docsnap);
            if (snapshot.exists()) {
    ...
    useEffect(() => {
        const fetchfollowing = async () => {
            var useruid=localStorage.getItem('clickeduid');
            const Uid = auth.currentUser.uid;
            const docsnap = doc(db, 'Following', useruid);
            const snapshot = await getDoc(docsnap);
            if (snapshot.exists()) {
